...
- [x] **8 — Verify + MERGE to `main` — DONE 2026-07-19.** **8a** headless end-to-end (Playwright chromium): the app pinned to 1840 flows correctly (69 vessels, era label "the long peace", 0 console errors); `silences.html` renders all 23 entries; a Node full-cycle probe confirms all 9 new ports receive traffic and every new system folds + spawns (coerced systems ride their commercial lanes per the record model, framing intact); the research-page `/favicon.ico` 404 fixed via `nav.js`. **8b** docs — CLAUDE.md/AGENTS.md record Phase 1 complete (per-port sourcing lives in the `research/flows/*-authoring.md` dossiers). **Merged `phase-1-world-build` → `main`** (no-ff): `main` now runs 1550→1850, 105 ports, 414 routes, 55 tests green. **PHASE 1 IS COMPLETE.**
